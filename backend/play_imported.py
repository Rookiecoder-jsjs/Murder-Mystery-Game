...
"""Play an imported story - matching original CLI style."""
...
class MurderMysteryCLI:
    ...
    def _show_clue_board(self):
        ...
        board = self.session.game.get_clue_board(self.session.human_player_id)

        print(Fore.YELLOW + "\n【线索看板】")

        if board.owned:
            print(Fore.GREEN + "  已获得线索:")
            for entry in board.owned:
                print(f"    - {entry.clue.content}")
        else:
            print(Fore.WHITE + "  已获得线索: 无")

        if board.scene_public:
            print(Fore.CYAN + "  场景公开线索:")
            for entry in board.scene_public:
                print(f"    - {entry.clue.content}")
    ...
    async def run(self):
        ...
        # 显示选项
        print(Fore.YELLOW + "\n请选择:")
        print(Fore.YELLOW + "  1. 加载已有故事")
        print(Fore.YELLOW + "  2. 从文件导入故事")
        ...
        if choice == "1":
            ...
            try:
                ...
                if 0 <= idx < len(stories):
                    ...
                    self.archive = load_story(story_id)
                    if not self.archive:
                        print(Fore.RED + "加载失败!")
                        return
            except ValueError:
        ...
        print(Fore.WHITE + f"{self.archive.case.background[:100]}...")

        print(Fore.YELLOW + "\n请选择你要扮演的角色ID: ")
        for char in self.archive.characters:
            killer_mark = " [凶手]" if char.is_killer else ""
            print(Fore.GREEN + f"  {char.id}. {char.name}（{char.public_identity}）{killer_mark}")

        char_id = input(Fore.YELLOW + "\n角色ID: ").strip()
        if char_id not in [c.id for c in self.archive.characters]:
            print(Fore.RED + "无效的角色ID！")
            return
            ...
            if user_input:
                ...
                for char_id, ai in self.session.ai_characters.items():
                    ...
                    response = ai.respond(
                        ...
                        other_chars=[
                            c for c in self.session.archive.characters if c.id != char_id
                        ],
